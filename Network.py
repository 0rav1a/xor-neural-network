...
ERROR = 0.001 #Margen de error de la red a partir del cual se considerará entrenada
    ...
    def __init__(self, nNeurons):
        ...
        for l in range(len(nNeurons))[1:-1]:
            for _ in range(nNeurons[l]): #Capas hidden
                self.layers[l].append(Neuron(self.layers[l-1]+[self.bias], self.layers[l+1]))
                
        for _ in range(nNeurons[-1]): #Capa output
            self.layers[-1].append(Neuron(self.layers[-2]+[self.bias], []))
