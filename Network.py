...
class Network:
    def __init__(self, shape):
        '''
        shape: Lista que define la topología de la red. La longitud será el número de capas, y el valor el número de neuronas en cada capa
        '''
        ...
        self.biases = [np.random.normal(size=(1, n)) for n in shape] #Bias de cada capa [capa][peso][0] (los de la capa 0 no se utilizan)
    ...
    def calcOutputs(self, inputs):
        ...
        for l in range(len(self.outputs))[1:]: #Feedforward: Desde la primera capa hasta la última
...
def softmax(z): return np.exp(z)/np.sum(np.exp(z)) #(0, 1] La suma es 1
def dsoftmax(z):
    tmp = softmax(z)
    return tmp*(1-tmp)

#FUNCIONES DE COSTE
#a: Vector de activaciones de la última capa
#y: Vector de salidas esperadas de la última capa
#return: Coste (escalar) de toda la red para el test dado
#return (derivative): Vector de derivadas de la función de coste respecto a la salida de cada neurona
